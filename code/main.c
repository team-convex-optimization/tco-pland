...
static void frame_test_injector(uint8_t *pixel_dest, int length, void *args_ptr)
{
  ...
  for (uint16_t y = 0; y < TCO_SIM_HEIGHT; y++)
  {
    ...
    {
      /* Fraction from upper left corner along the diagonal (1 = bottom right corner, 0 = top left
      corner). */
...
static void frame_raw_processor(uint8_t *pixels, int length, void *args_ptr)
{
  ...
  /* This is an array which holds a copy of the processed frame. This is done because nothing can be
  guaranteed about the 'pixels' pointer (it could even be read-only). Needs to be aligned in order
  for memcpy to be used. */
...
int main(int argc, char *argv[])
{
  ...
  {
    /* Run the pipeline which reads simulated camera data from 'tco_sim' and processes it using the
    function as it would when running the real camera pipeline. 
