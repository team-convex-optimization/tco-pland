...
void find_edges_scan(uint8_t (*pixels)[TCO_SIM_HEIGHT][TCO_SIM_WIDTH], uint16_t center_width, uint16_t height, uint16_t (*edges)[2])
{
    for (uint16_t i = center_width; i < TCO_SIM_WIDTH - SEGMENTATION_DEADZONE; i++)
    {
...
void plot_vector_points(uint8_t (*pixels)[TCO_SIM_HEIGHT][TCO_SIM_WIDTH])
{
    ...
    const uint16_t target_lines[NUM_VECTOR_POINTS] = { //TODO : Dynamically initialize me.
        (28 * TCO_SIM_HEIGHT)  / 72,  /* Closest to the car */
        (27 * TCO_SIM_HEIGHT)  / 72,
        (26 * TCO_SIM_HEIGHT)  / 72,
        (25 * TCO_SIM_HEIGHT)  / 72,
        (24 * TCO_SIM_HEIGHT)  / 72,
        (23 * TCO_SIM_HEIGHT)  / 72,
        (22 * TCO_SIM_HEIGHT)  / 72,
        (21 * TCO_SIM_HEIGHT)  / 72,
        (20 * TCO_SIM_HEIGHT)  / 72,
        (19 * TCO_SIM_HEIGHT)  / 72,
        (18 * TCO_SIM_HEIGHT)  / 72,
        (17 * TCO_SIM_HEIGHT)  / 72,
        (16 * TCO_SIM_HEIGHT)  / 72,  
        (15 * TCO_SIM_HEIGHT)  / 72,
        (14 * TCO_SIM_HEIGHT)  / 72,
        (13 * TCO_SIM_HEIGHT)  / 72,
        (12 * TCO_SIM_HEIGHT)  / 72,
        (11 * TCO_SIM_HEIGHT)  / 72,
        (10 * TCO_SIM_HEIGHT)  / 72,
        (9 * TCO_SIM_HEIGHT)   / 72,
        (8 * TCO_SIM_HEIGHT)   / 72,
        (7 * TCO_SIM_HEIGHT)   / 72,
        (6 * TCO_SIM_HEIGHT)   / 72,
    ...
    for (uint16_t e = 0; e < 2; e++)
    {
            if (vectors[e].valid == 0)
                break;
            plot_line(pixels, vectors[e].bot.x, vectors[e].bot.y, vectors[e].top.x, vectors[e].top.y, 64);
            plot_square(pixels, vectors[e].bot.x, vectors[e].bot.y, 8, 32);
            plot_square(pixels, vectors[e].top.x, vectors[e].top.y, 8, 32);
    }
    #endif
